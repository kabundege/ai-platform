...
import { AiOutlineArrowDown, AiOutlineArrowLeft, AiOutlineArrowUp, AiOutlineLoading3Quarters } from 'react-icons/ai';
...
const WeatherComponent = ({label,value,Icon}:{label:string,value:string|number|ReactNode,Icon:ReactNode}) => (
...
export default function FarmOne({name,index}:{name:string,index:number}) {
    ...
    useEffect(()=>{
        const getWeither = () => {
            navigator.geolocation.getCurrentPosition((position)=>{
                ...
                // 
                fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&metric=celsius&imperial=celsius&appid=${API_key}`)
                .then(res => res.json())
                .then(res => {
                    if(res.cod === 200)
                    setWeather(res)
    ...
    const ModalView = farmWeather ? (
        <div className="w-[460px] text-left">
            <AiOutlineArrowLeft className='text-borange text-lg cursor-pointer hover:opacity-80' onClick={toggleModal} />
            ...
            <div className='flex justify-between items-center mb-5'>
                ...
                <p className='text-base text-bash font-normal'> {farmWeather.sys.country} / {farmWeather.name}</p>
            </div>
            ...
            <div className='flex justify-between items-center mt-5'>
                <Button onClick={toggleActive} text={isActive?"De-activate":"Activate"} className='bg-whitesmoke text-black border' />
                <Button onClick={toggleDelete} text="Delete" className='bg-bred text-white ml-5 border border-whitesmoke' />
            </div>
        </div>
    ) : (
        <>
