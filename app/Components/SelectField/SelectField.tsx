...
import { errorStyle, flexer } from '../../../styles/comon.styles';
...
const SelectField:FC<Props> = ({ error,value,wrapperClassName,labelClassName,showAcheck,onChange,data,placeholder,className, disabled=false,showSearch = false }) => {
  ...
  return (
    <div className={"w-full flex my-3 relative flex-col "+className} ref={selectRef}>
      <div className={flexer+`px-4 py-3 cursor-pointer border ${error?'border-red-300':""} bg-gray-100 rounded-lg ${disabled && 'bg-gray-200'} mt-1 ${wrapperClassName}`} onClick={toggleModal}>
        {
          localValue ?
              ...
              React.Children.toArray(
                customData.map(({ value,label },index) => 
                  showAcheck ? (
                    ...
                    <p 
                      className={`px-5 py-1 rounded-md w-full cursor-pointer hover:bg-gray-200 hover:text-gray-800 ${index ? null : 'mt-1'} ${labelClassName}`}
                      onClick={() => handleChange(value,label)} 
                    >
                      {label || value}
                    </p> 
